...
Paddle 在 Python 层已逐步对齐 PyTorch,但**深度学习编译生态中大量第三方 C++ 库**(FastDeploy 推理引擎、DeepEP(含 hybrid-ep 分支) 通信库、DeepGEMM / FlashMLA 等高性能 kernel、paddlecodec 等多模态算子库)依赖 **libtorch C++ API**(`ATen` + `c10` + `torch::`)直接编译。如果 Paddle 不提供等价的 C++ API 表面,这些库就只能选择 PyTorch,Paddle 生态被卡在"Python 用户友好但 C++ 集成不友好"。
